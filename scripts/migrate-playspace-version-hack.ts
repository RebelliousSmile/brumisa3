...
async function main() {
  console.log('Migration systemId vers versionId + hackId')
  ...
  console.log(`   ${comUpdated.count} playspaces migres`)
  ...
  console.log(`   ${litmUpdated.count} playspaces migres`)
  ...
  console.log(`versionId NULL : ${nullVersion} playspaces`)

  const nullHack = await prisma.playspace.count({
    where: { hackId: null }
  })
  console.log(`hackId NULL : ${nullHack} playspaces`)

  // Validation finale
  console.log('\n' + '=' .repeat(50))

  if (nullVersion > 0 || nullHack > 0) {
    console.error('ERREUR : Certains playspaces ont versionId ou hackId NULL')
    process.exit(1)
  }

  if (v1Count + v2Count !== totalBefore) {
    console.error(`ERREUR COUNT : avant=${totalBefore}, apres=${v1Count + v2Count}`)
    process.exit(1)
  }

  console.log('MIGRATION REUSSIE : Tous playspaces migres correctement')
  console.log('=' .repeat(50))
}
